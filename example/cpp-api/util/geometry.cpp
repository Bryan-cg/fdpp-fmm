...
GEOM::calculate_cos_theta(const Point &p1_edge, const Point &p2_edge, const Point &p1_trace, const Point &p2_trace) {
    ...
    double az_edge = bg::formula::spherical_azimuth(p1_edge.get<0>() * d2r, p1_edge.get<1>() * d2r,
                                                    p2_edge.get<0>() * d2r, p2_edge.get<1>() * d2r);
    double az_trace = bg::formula::spherical_azimuth(p1_trace.get<0>() * d2r, p1_trace.get<1>() * d2r,
                                                     p2_trace.get<0>() * d2r, p2_trace.get<1>() * d2r);
...
void
GEOM::calc_accuracy(const LineStringDeg &trace, const LineStringDeg &match, double *li, double *avgA, double *frechet,
                    double *hausdorff) {
    double length_match = bg::length(match, Haversine);
    double length_trace = bg::length(trace, Haversine);
    if (length_match == 0 || length_trace == 0) {
        *frechet = 0.0;
        *hausdorff = 0.0;
        *li = 0.0;
        *avgA = 0.0;
    ...
    *frechet = bg::discrete_frechet_distance(match, trace, Haversine);
    *hausdorff = bg::discrete_hausdorff_distance(match, trace, Haversine);
    *li = length_match / length_trace;
